...
size_t render_value(size_t sx, size_t sy, size_t ex, size_t ey, value_getter vget);
...
size_t render_element(struct dspelement *menu, size_t i, size_t menu_size, size_t w, size_t h, size_t sw, size_t *prev_y) {
	...
	switch(menu[i].type) {
		...
		case EL_GRAPH_OVER_TIME:
			r = render_graph_over_time(sw, *prev_y, w, i * h / menu_size, &menu[i].args.v, menu[i].func);
			*prev_y = i * h / menu_size;
			break;
		case EL_DIVIDER:
